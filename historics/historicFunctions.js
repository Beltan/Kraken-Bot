...
exports.getHistoric = function() {
    if (fs.existsSync('./data/LastID' + getHistoric.pair + '.txt')){
        getHistoric.last = fs.readFileSync('./data/LastID' + getHistoric.pair + '.txt', 'utf8');
    }
    functions.historic();
}
...
exports.printResultsHistoric = function(error, data) {
    if(error != null){
        ...
        tempData.unshift('');
        var pairName = './data/' + getHistoric.pair + '.csv';
        var lastID = './data/LastID' + getHistoric.pair + '.txt';
