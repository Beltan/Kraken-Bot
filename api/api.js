...
let cancelOrder = async function(decision) {
    ...
    try {
        ...
        logger.debug('Order ' + txid + ' canceled succesfully');
        return order;
    } catch (e) {
        logger.error('Error while canceling order, order not canceled. Trying again... -> ' + e);
    }
}
...
let placeOrder = async function(decision) {
    ...
    try {
        ...
        logger.debug(type + ' order placed succesfully -> ' + type + ' ' + volume + ' ' + pair + ' @ ' + ordertype + ' ' + price);
        return order;
    }
    catch (e) {
